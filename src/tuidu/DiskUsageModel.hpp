...
/// Adapts a disk-usage @ref Tree to the generic @c core::tui::TreeTableModel so the reusable
/// @c core::tui::TreeTableView renders and navigates it. This is the DI seam between the
...
class DiskUsageModel: public core::tui::TreeTableModel
{
  public:
    /// @param tree The disk-usage tree to present (not owned; outlives the model).
    explicit DiskUsageModel(Tree& tree) noexcept;

    // --- core::tui::TreeTableModel ---
    [[nodiscard]] std::vector<core::tui::TableColumn> columns() const override;
    [[nodiscard]] std::vector<core::tui::RowId> rows() const override;
    [[nodiscard]] std::string cellText(core::tui::RowId row, std::size_t column) const override;
    [[nodiscard]] core::tui::RowStyle rowStyle(core::tui::RowId row, bool selected) const override;
    [[nodiscard]] bool canDescend(core::tui::RowId row) const override;
    bool descend(core::tui::RowId row) override;
